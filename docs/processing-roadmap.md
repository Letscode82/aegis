...
| **PROC-6** | Container expansion (PST/MBOX/ZIP) | Ingest exported archives, nested families | pure-JS (jszip + MBOX parser + `pst-extractor`) | 🟡 | ✅ **ZIP + MBOX + PST shipped** (`ingestArchive`, workspace upload → review set); large-archive worker path (>~3.5 MB inline) pending |
