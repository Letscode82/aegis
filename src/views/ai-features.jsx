...
import { Pill, Card } from "../atoms/ui";
import { AIInsight, useAIInsight } from "../atoms/ai-insight";
 ...
 * Pure function — no React hooks. The MissionControlView passes this
 * its own KPI data; we layer in cross-cutting aggregates.
 *
 * @param {object} input
 ...
 * @returns {object}
 */
export function buildBriefingContext({ domains, posture }) {
  const today = new Date();
  const dateLabel = today.toLocaleDateString("en-US", { weekday: "long", month: "long", day: "numeric", year: "numeric" });
  const criticalAlerts = ALL_ALERTS.filter(a => a.sev === "critical").length;
  const pendingApprovals = ALL_APPROVALS.length;
