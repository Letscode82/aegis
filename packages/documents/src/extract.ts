/**
 * Document text extraction — shared (@aegis/documents).
 *
 * Pure — no DB, no network. Turns an uploaded .txt, .docx, or .pdf buffer into
 * plain text. Used by intake document upload AND contracts third-party-paper
 * intake. Legacy .doc (binary) is out of scope.
 *
 * .docx is a ZIP whose body text lives in `word/document.xml`. We parse the ZIP
 * via its central directory (accurate sizes + offsets), inflate the one entry we
 * need with Node's zlib, and strip the WordprocessingML down to text.
 * Dependency-free on purpose — no jszip/mammoth to install.
 *
 * .pdf: we extract text from the page content streams (FlateDecode-inflated
 * where needed) by interpreting the text-showing operators (Tj / TJ / ' / ").
 * This covers text-based PDFs (Word/Docs "Save as PDF", LaTeX, etc.).
 * Scanned/image-only PDFs and exotic CID-font subsets won't yield text — we
 * detect that and fail with an honest message. A full pdf.js swap is a
 * hardening-phase upgrade.
 */
 ...
 * UnsupportedDocumentFormatError for unsupported types and DocumentParseError
 * when a supported file can't be parsed.
 */
